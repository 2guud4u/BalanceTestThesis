...
from dataloader import  get_dino_dataset, get_label_mapping
...
if __name__ == "__main__":
    ...
    features,labels, fps_list=get_dino_dataset(train_set_path, size=512)
    aug_features, aug_labels, aug_fps_list = get_dino_dataset(train_set_path, size=512, augmented=True)
    ...
    aug_X_train = torch.cat([features[i] for i in train_indices], dim=0)
    X_train = torch.cat([X_train, aug_X_train], dim=0)

    y_train = torch.cat([labels[i] for i in train_indices], dim=0)
    aug_y_train = torch.cat([labels[i] for i in train_indices], dim=0)
    y_train = torch.cat([y_train, aug_y_train], dim=0)
    ...
    class_weights = compute_class_weight('balanced', classes=np.array([0,1,2,3,4]), y=y_train.numpy())
    ...
    model = TwoLayerDinoNetwork().to(device)
