...
from dataloader import  get_dino_dataset, create_val_train_splits, get_label_mapping, get_dataset
...
if __name__ == "__main__":
    ...
    y_train = (y_train == 3).long()  # Assuming 0 is the positive class for phase one
    # Convert to tensor
    y_train = y_train.to(torch.float32)
    # Select val videos
    X_val = torch.cat([features[i] for i in val_indices], dim=0)
    y_val = torch.cat([labels[i] for i in val_indices], dim=0)
    y_val = (y_val == 3).long()  # Assuming 0 is the positive class for phase one
    # Convert to tensor
    ...
    disp = ConfusionMatrixDisplay(confusion_matrix=cm, display_labels=["phase 4", "Not phase 4"])
    ...
    with open(saveDir+"classification_report.txt", 'w') as f:
        f.write(classification_report(truths, preds, labels=[1, 0],target_names=["phase 4", "Not phase 4"]))
