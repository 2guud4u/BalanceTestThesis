...
if __name__ == "__main__":
    ...
    y_train = (y_train == 1).long()  # Assuming 0 is the positive class for phase one
    # Convert to tensor
    y_train = y_train.to(torch.float32)
    # Select val videos
    X_val = torch.cat([features[i] for i in val_indices], dim=0)
    y_val = torch.cat([labels[i] for i in val_indices], dim=0)
    y_val = (y_val == 1).long()  # Assuming 0 is the positive class for phase one
    # Convert to tensor
    ...
    disp = ConfusionMatrixDisplay(confusion_matrix=cm, display_labels=["phase 2", "Not phase 2"])
    ...
    with open(saveDir+"classification_report.txt", 'w') as f:
        f.write(classification_report(truths, preds, labels=[1, 0],target_names=["phase 2", "Not phase 2"]))
