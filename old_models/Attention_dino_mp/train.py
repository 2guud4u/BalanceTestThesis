...
from network import Dino_MP_Attention_Network
from dataloader import  get_dino_dataset, create_val_train_splits, get_world_pose_dataset, get_dataset
...
if __name__ == "__main__":
    ...
    dino_features,labels, fps_list=get_dataset(train_set_path, "dinov3_features",frame_aware=False)

    mp_features,_ = get_world_pose_dataset(train_set_path)
    ...
    train_mp = torch.cat([mp_features[i] for i in train_indices], dim=0)
    train_labels = torch.cat([labels[i] for i in train_indices], dim=0)


    combined_train_data = list(zip(train_dino, train_mp, train_labels))


    val_dino = torch.cat([dino_features[i] for i in val_indices], dim=0)
    val_mp = torch.cat([mp_features[i] for i in val_indices], dim=0)
    val_labels = torch.cat([labels[i] for i in val_indices], dim=0)
    combined_val_data = list(zip(val_dino, val_mp, val_labels))
    #class_weights
    ...
    model = Dino_MP_Attention_Network().to(device)
