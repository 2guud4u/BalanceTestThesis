...
class Simple_MSTCN2_Trainer:
    def __init__(self, encoder, num_layers_PG=11, num_layers_R=10, num_R=3, num_f_maps=64, num_classes=2, class_weights=None, early_stop_patience=30, early_stop_min_delta=0.0,
                 time_alignment="upsample_preds", early_stop_monitor="val_f1"):
        """
        Args:
        ...
        self.model = MS_TCN2(num_layers_PG, num_layers_R, num_R, num_f_maps, encoder.out_dim, num_classes)
    ...
    def _early_stop_step(self, val_metric):
        ...
        if higher_is_better:
            improved = (self._best_val is None or
                        val_metric > (self._best_val + self.early_stop_min_delta))
        else:
            improved = (self._best_val is None or
                        val_metric < (self._best_val - self.early_stop_min_delta))
    ...
    def train(self, save_dir, batch_gen, num_epochs, batch_size, device, learning_rate=0.0005, val_batch_gen=None, lambda_smooth=0.15, weight_decay=0.0):
        self.model.train()
        ...
        trainable_params = [p for p in list(self.encoder.parameters()) + list(self.model.parameters()) if p.requires_grad]
        ...
        for epoch in range(num_epochs):
            ...
            while batch_gen.has_next():
                ...
                for p in predictions_aligned:
                    loss += self.ce(p.transpose(2, 1).contiguous().view(-1, self.num_classes), target_aligned.view(-1))
                    loss += lambda_smooth*torch.mean(torch.clamp(self.mse(F.log_softmax(p[:, :, 1:], dim=1), F.log_softmax(p.detach()[:, :, :-1], dim=1)), min=0, max=16)*mask_aligned[:, :, 1:])
                ...
                correct += ((predicted == target_aligned).float()*mask_aligned[:, 0, :].squeeze(1)).sum().item()
            ...
            _, train_f1 = compute_f1(
                torch.cat(train_all_preds), torch.cat(train_all_targets),
                torch.cat(train_all_masks), self.num_classes
            )
            ...
            if val_batch_gen is not None:
                ...
                with torch.no_grad():
                    while val_batch_gen.has_next():
                        val_input, val_target, val_mask = val_batch_gen.next_batch(batch_size)
                        val_input, val_target, val_mask = val_input.to(device), val_target.to(device), val_mask.to(device)
                        
                        val_encoder_features = self.encoder(val_input)  # (B, D, T)
                        val_predictions = self.model(val_encoder_features)  # list[(B, C, T)]
                        
                        val_preds_aligned, val_target_aligned, val_mask_aligned = self.aligner(
                            val_predictions, val_target, val_mask
                        )
                        
                        val_loss_batch = 0
                        for p in val_preds_aligned:
                            val_loss_batch += self.ce(p.transpose(2, 1).contiguous().view(-1, self.num_classes), val_target_aligned.view(-1))
                            val_loss_batch += lambda_smooth*torch.mean(torch.clamp(self.mse(F.log_softmax(p[:, :, 1:], dim=1), F.log_softmax(p.detach()[:, :, :-1], dim=1)), min=0, max=16)*val_mask_aligned[:, :, 1:])
                        
                        val_epoch_loss += val_loss_batch.item()
                        num_val_batches += 1

                        _, val_predicted = torch.max(val_preds_aligned[-1], 1)
                        val_correct += ((val_predicted == val_target_aligned).float()*val_mask_aligned[:, 0, :].squeeze(1)).sum().item()
                ...
                val_batch_gen.reset()
                val_loss = val_epoch_loss / num_val_batches if num_val_batches > 0 else 0.0
                val_acc = val_correct / val_total if val_total > 0 else 0.0
                _, val_f1 = compute_f1(
                    torch.cat(val_all_preds), torch.cat(val_all_targets),
                    torch.cat(val_all_masks), self.num_classes
                )
                self.encoder.train()
                self.model.train()
            ...
            if val_batch_gen is not None:
                ...
                higher_is_better = self.early_stop_monitor == "val_f1"
                is_best = (self._best_saved is None or
                           (higher_is_better and es_metric > self._best_saved) or
                           (not higher_is_better and es_metric < self._best_saved))
                if is_best:
                    ...
                    logger.info(f"  ↳ Saved best model ({self.early_stop_monitor}={es_metric:.4f})")

            # -------- Logging --------
            train_loss = epoch_loss / num_train_batches if num_train_batches > 0 else 0.0
            if val_loss is None:
                logger.info("[epoch %d]: train_loss = %f,   train_acc = %f,   train_f1 = %f" % (epoch + 1, train_loss, train_acc, train_f1))
            else:
                logger.info("[epoch %d]: train_loss = %f,   train_acc = %f,   train_f1 = %f,   val_loss = %f,   val_acc = %f,   val_f1 = %f" % (epoch + 1, train_loss, train_acc, train_f1, val_loss, val_acc, val_f1))
    ...
    def predict(self, model_dir, results_dir, features_path, vid_list_file, actions_dict, device, sample_rate, checkpoint_name="best"):
        """
        ...
        with torch.no_grad():
            ...
            for vid in list_of_vids:
                ...
                features = features[:, ::sample_rate] if features.shape[0] < features.shape[1] else features[::sample_rate, :]
