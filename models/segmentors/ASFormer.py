"""
ASFormer — original source code from Yi *et al.*, "ASFormer: Transformer
for Action Segmentation" (BMVC 2021).

Source: https://github.com/ChinaYi/ASFormer

Minimal adaptations for this codebase:
  - Removed global `device` variable; tensors are created on the same
    device as the input.
  - Removed bundled Trainer / eval import (this codebase has its own).
  - Added `ASFormerSegmentor` wrapper whose forward(x) matches the
    segmentor contract: (B, D, T) → (S, B, C, T), without requiring
    a mask argument.  Internally it passes an all-ones mask to
    `MyTransformer` so the original attention logic is unchanged.
"""
...
class AttLayer(nn.Module):
    ...
    def _normal_self_att(self,q,k,v, mask):
        ...
        _,c2,L = k.size()
        _,c3,L = v.size()
        padding_mask = torch.ones((m_batchsize, 1, L), device=q.device) * mask[:,0:1,:]
    ...
    def _block_wise_self_att(self, q,k,v, mask):
        ...
        nb = L // self.bl
        if L % self.bl != 0:
            q = torch.cat([q, torch.zeros((m_batchsize, c1, self.bl - L % self.bl), device=q.device)], dim=-1)
            k = torch.cat([k, torch.zeros((m_batchsize, c2, self.bl - L % self.bl), device=k.device)], dim=-1)
            v = torch.cat([v, torch.zeros((m_batchsize, c3, self.bl - L % self.bl), device=v.device)], dim=-1)
            nb += 1

        padding_mask = torch.cat([torch.ones((m_batchsize, 1, L), device=q.device) * mask[:,0:1,:], torch.zeros((m_batchsize, 1, self.bl * nb - L), device=q.device)],dim=-1)
    ...
    def _sliding_window_self_att(self, q,k,v, mask):
        ...
        k = torch.cat([torch.zeros(m_batchsize, c2, self.bl // 2, device=k.device), k, torch.zeros(m_batchsize, c2, self.bl // 2, device=k.device)], dim=-1)
        v = torch.cat([torch.zeros(m_batchsize, c3, self.bl // 2, device=v.device), v, torch.zeros(m_batchsize, c3, self.bl // 2, device=v.device)], dim=-1)
        padding_mask = torch.cat([torch.zeros(m_batchsize, 1, self.bl // 2, device=q.device), padding_mask, torch.zeros(m_batchsize, 1, self.bl // 2, device=q.device)], dim=-1)
...
# ──────────────────────────────────────────────────────────────────────
# Wrapper for this codebase's segmentor contract
# ──────────────────────────────────────────────────────────────────────

class ASFormer(nn.Module):
    """
    Thin wrapper around the original MyTransformer so it matches the
    segmentor contract used by SegmentorWithLoss / Trainer:

        forward(x)  →  (S, B, C, T)

    Internally creates an all-ones mask (padding is handled at the loss
    level via ignore_index=-100).

    Args:
        dim:                  input feature dim (encoder.out_dim).
        num_classes:          number of segmentation classes.
        num_f_maps:           internal feature map size.
        num_layers:           number of dilated attention layers per stage.
        num_decoders:         number of refinement decoder stages.
    ...
    def forward(self, x):
        """
        Args:
            x: (B, D, T) — encoder features in channel-first layout.

        Returns:
            (S, B, C, T) — stacked stage logits where S = 1 + num_decoders.
        """
        # Create all-ones mask on the same device as x.
        # Shape: (B, 1, T) — matches what MyTransformer expects.
